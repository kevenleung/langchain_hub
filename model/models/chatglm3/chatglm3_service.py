...
from models.chatglm3.chatglm3_utils import process_response, generate_chatglm3, generate_stream_chatglm3
...
async def get_embeddings(request: EmbeddingRequest):
    embeddings = [Embbeding().model.encode(text) for text in request.input]
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    response = generate_chatglm3(llm.model, llm.tokenizer, gen_params)
...
async def predict(model_id: str, params: dict):
    ...
    previous_text = ""
    for new_response in generate_stream_chatglm3(llm.model, llm.tokenizer, params):
...
def predict_stream(model_id, gen_params):
    ...
    llm = ChatGLM3()
    for new_response in generate_stream_chatglm3(llm.model, llm.tokenizer, gen_params):
        decoded_unicode = new_response["text"]
...
# 开启llm服务
def start_model_server(host, port):
    is_ok = True
    try:
